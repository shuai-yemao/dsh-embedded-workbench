...
| 当前状态 | `pass`：4 个 fake-clock 主机测试、UI package `tsc --noEmit` 通过。Controller 仅保存当前快照、loading/error、一个 timer 与订阅；瞬态轮询为 500 ms、最多 20 次，Settings/connection disposer 与 in-flight 操作在 dispose 时对称释放。 |
